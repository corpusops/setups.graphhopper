...
## graphhopper settings setup
To add/modify graphhopper settings, you specifically need to edit ``.ansible/playbooks/roles/graphhopper_vars/defaults/main.yml`` for hosted deployments,<br/>
and adapt certainly one of ``cops_graphhopper_docker_env_freeform``, ``cops_graphhopper_docker_env_defaults``, ``cops_graphhopper_docker_env_extra``.
  - Here, never forget to abuse ansible variables to cut variables from their templates<br/>
    ...
    eg:
      - In all cases, adapt/edit inventory (variables may already exist)
        ```sh
        $EDITOR .ansible/playbooks/roles/graphhopper_vars/defaults/main.yml
        ```

          ```yaml
          # ...
          cops_graphhopper_docker_env_extra:
            MYSETTINGS={{cops_graphhopper_foobar}}
            MYSECRETSETTINGS={{cops_graphhopper_footruc}}
          ```
      - Set variables values in related clear group vars
        ...
        ```
          ...
          ```yaml
          # ...
          cops_graphhopper_foobar: supervalue
          ```
      ...
      - Set crypted values (see ansible/README about vaults) in related crypted group vars
        ...
        ```
          ...
          ```yaml
          # ...
          cops_graphhopper_footruc: abcd123456789secret
          ```
